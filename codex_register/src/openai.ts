...
export class OpenAIClient {
    ...
    private async fetchWithRetry(
        ...
        for (let attempt = 1; attempt <= FETCH_RETRY_COUNT; attempt++) {
            ...
            try {
                return await baseFetch(input, timed.init);
            } catch (error) {
                const reportedError = timed.didTimeout()
                    ? withCause(new Error(`request timeout after ${timeoutMs}ms`), error)
                    : error;
                lastError = reportedError;
                if (!isRetryableFetchError(reportedError) || attempt >= FETCH_RETRY_COUNT) {
                    throw reportedError;
                }
                console.log(
                    `[网络重试 ${attempt}/${FETCH_RETRY_COUNT}] ${this.describeRetryTarget(input)} ${this.describeRetryError(reportedError)}`,
                );
