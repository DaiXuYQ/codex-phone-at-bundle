...
- `GET /api/accounts` — account ledger view. Links phone, AT hash, Plus job, bound email, and OA target/account.
- `GET /api/accounts/{id}` / `GET /api/accounts/by-token/{hash}` / `GET /api/accounts/by-phone/{phone}` / `GET /api/accounts/by-email/{email}` — ledger lookup.
- `POST /api/reconcile` — rebuild/link the account ledger from existing register tasks, Plus jobs, OA status, and the AT pool.
- `POST /api/workflows/phone-plus-oa` — one-click orchestration for phone register -> optional Plus upgrade -> OA binding.
- `GET /api/workflows` / `GET /api/workflows/{runId}` — workflow status and logs.
- `POST /api/workflows/{runId}/plus-otp` — submit Plus OTP and resume the workflow.
- `POST /api/workflows/{runId}/resume` / `POST /api/workflows/{runId}/cancel` — resume or cancel a workflow.
...
```json
POST /api/workflows/phone-plus-oa
{
  "target": "sub2api",
  "plus": true,
